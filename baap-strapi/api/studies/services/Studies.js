...
module.exports = {
  ...
  add: async (values) => {
    ...
    return Studies.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  /**
   * Promise to search a/an studies.
   *
   * @return {Promise}
   */
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Studies.attributes).reduce((acc, curr) => {
      switch (Studies.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
